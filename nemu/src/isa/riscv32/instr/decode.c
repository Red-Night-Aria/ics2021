...
static def_DHelper(J) {
  ...
  sword_t simm = (s->isa.instr.j.imm10_1 << 1) | (s->isa.instr.j.imm11 << 11) |
                 (s->isa.instr.j.imm19_12 << 12) | (s->isa.instr.j.simm20 << 20);
...
static def_DHelper(B) {
  sword_t simm = (s->isa.instr.b.imm4_1 << 1) | (s->isa.instr.b.imm10_5 << 5) | 
                 (s->isa.instr.b.imm11 << 11) | (s->isa.instr.b.imm12 << 12);
...
// after lookup in table<main>, do a second lookup to decide which sub-load instr the input is. 
def_THelper(load) {
...
def_THelper(rr_compute) {
  ...
  def_INSTR_TAB("0000001 ????? ????? 110 ????? ????? ??", remu);
  return EXEC_ID_inv;
}

def_THelper(branch_jump) {
  // 0b1 100 00001 10001 11
...
def_THelper(main) {
  ...
  def_INSTR_IDTAB("??????? ????? ????? ??? ????? 00100 11", I     , ri_compute);
